...
namespace TradeMaster6000.Server.DataHelpers
{
    public class TickDbHelper : ITickDbHelper
    {
        ...
        public async Task<List<MyTick>> Get(uint token, DateTime time)
        {
            using (var context = contextFactory.CreateDbContext())
            {
                var ticks = context.Ticks.Where(x => x.InstrumentToken == token && x.Timestamp.Day == time.Day && x.Timestamp.Hour == time.Hour && x.Timestamp.Minute == time.Minute);
                return await ticks.OrderBy(x => x.Timestamp).ToListAsync();
            }
        }
